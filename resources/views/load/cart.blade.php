													...
													@foreach(Session::get('cart')->items as $product)
														<li class="product cremove{{ $product['item']['id'].$product['size'].$product['color'].str_replace(str_split(' ,'),'',$product['values']) }}">
															<div class="product-details">
																<div class="content">
																	<a href="{{ route('front.product',$product['item']['slug']) }}"><h4 class="product-title">{{$lang ? (mb_strlen($product['item']['name_fa'],'utf-8') > 45 ? mb_substr($product['item']['name_fa'],0,45,'utf-8').'...' : $product['item']['name_fa']) : (mb_strlen($product['item']['name'],'utf-8') > 45 ? mb_substr($product['item']['name'],0,45,'utf-8').'...' : $product['item']['name'])}}</h4></a>
